...
// include: C:/Users/Administrator/Desktop/rust/imnaa/platform/wasm/pre.js
Module['preRun'] = function () {
    FS.createPreloadedFile('/', 'data01.pkg', 'data01.pkg', true, false);
};
// end include: C:/Users/Administrator/Desktop/rust/imnaa/platform/wasm/pre.js
...
var ASM_CONSTS = {
  486776: () => { if (window.navigator.userAgent.indexOf('iPad') != -1 || window.navigator.userAgent.indexOf('iPhone') != -1) { FS.syncfs(function (err) {}); } else { FS.syncfs(function (err) { alert('Saved!'); }); } },  
 486979: () => { if (window.navigator.userAgent.indexOf('iPad') != -1 || window.navigator.userAgent.indexOf('iPhone') != -1) { FS.syncfs(function (err) {}); } else { FS.syncfs(function (err) { alert('Saved!'); }); } },  
 487182: () => { FS.mkdir("imnaa"); FS.mount(IDBFS, {}, "imnaa"); FS.syncfs(true, function (err) { Module.ccall('main_continue', 'v'); }); },  
 487308: ($0) => { document.title = Module.UTF8ToString($0); },  
 487354: () => { onResizeWindow(); },  
 487376: () => { window.addEventListener('resize', onResizeWindow); document.addEventListener('visibilitychange', function () { if(document.visibilityState === 'visible') { Module.ccall('setVisible'); document.getElementById('canvas').focus(); } else if(document.visibilityState === 'hidden') { Module.ccall('setHidden'); } }); document.getElementById('canvas').addEventListener('mouseleave', function () { Module.ccall('mouseLeave'); }); },  
 487802: ($0) => { alert(UTF8ToString($0)); },  
 487831: ($0) => { alert(UTF8ToString($0)); },  
 487860: ($0) => { alert(UTF8ToString($0)); },  
 487889: ($0, $1) => { document.getElementById("canvas").style.display = "none"; var v = document.getElementById("video"); v.style.display = "block"; v.src = Module.UTF8ToString($0, $1); v.load(); v.addEventListener('ended', function() { document.getElementById("canvas").style.display = "block"; document.getElementById("video").style.display = "none"; }); v.play(); },  
 488238: () => { var c = document.getElementById("canvas"); c.style.display = "block"; var v = document.getElementById("video"); v.style.display = "none"; v.pause(); v.src = ""; v.load(); },  
 488409: () => { var v = document.getElementById("video"); return v.ended; },  
 488467: () => { var canvas = document.getElementById('canvas_holder'); const method = canvas.requestFullscreen || canvas.webkitRequestFullscreen || canvas.mozRequestFullScreen || canvas.msRequestFullscreen; if (method) method.call(canvas); onResizeWindow(); },  
 488713: () => { const method = document.exitFullscreen || document.webkitExitFullscreen || document.mozCancelFullScreen || document.msExitFullscreen; if (method) method.call(document); onResizeWindow(); }
};
function onResizeWindow() { var canvas = document.getElementById('canvas'); var cw = canvas.width; var ch = canvas.height; var aspect = cw / ch; var winw = window.innerWidth; var winh = window.innerHeight; var w = winw; var h = winw / aspect; if(h > winh) { h = winh; w = winh * aspect; } canvas.style.width = w + 'px'; canvas.style.height = h + 'px'; canvas.focus(); }
function get_lang_code() { if (window.navigator.language.startsWith("ja")) return 0; return 1; }

// Imports from the Wasm binary.
var _malloc = makeInvalidEarlyAccess('_malloc');
var _fflush = makeInvalidEarlyAccess('_fflush');
var _main_continue = Module['_main_continue'] = makeInvalidEarlyAccess('_main_continue');
var _setVisible = Module['_setVisible'] = makeInvalidEarlyAccess('_setVisible');
var _setHidden = Module['_setHidden'] = makeInvalidEarlyAccess('_setHidden');
var _mouseLeave = Module['_mouseLeave'] = makeInvalidEarlyAccess('_mouseLeave');
...
var _license_info = Module['_license_info'] = makeInvalidEarlyAccess('_license_info');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['main_continue'] != 'undefined', 'missing Wasm export: main_continue');
  assert(typeof wasmExports['setVisible'] != 'undefined', 'missing Wasm export: setVisible');
  assert(typeof wasmExports['setHidden'] != 'undefined', 'missing Wasm export: setHidden');
  assert(typeof wasmExports['mouseLeave'] != 'undefined', 'missing Wasm export: mouseLeave');
  ...
  assert(typeof wasmExports['license_info'] != 'undefined', 'missing Wasm export: license_info');
  _malloc = createExportWrapper('malloc', 1);
  _fflush = createExportWrapper('fflush', 1);
  _main_continue = Module['_main_continue'] = createExportWrapper('main_continue', 0);
  _setVisible = Module['_setVisible'] = createExportWrapper('setVisible', 0);
  _setHidden = Module['_setHidden'] = createExportWrapper('setHidden', 0);
  _mouseLeave = Module['_mouseLeave'] = createExportWrapper('mouseLeave', 0);
  ...
  _license_info = Module['_license_info'] = wasmExports['license_info'].value;
}
